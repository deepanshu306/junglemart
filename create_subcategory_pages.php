...
echo "<h1>Creating Subcategory Pages</h1>";
echo "<p>Found " . count($subcategories) . " subcategories:</p>";

foreach ($subcategories as $subcat) {
    // Create filename for this subcategory
    $filename = strtolower(str_replace([' ', '/', '(', ')', '&'], ['_', '_', '', '', 'and'], $subcat['name'])) . '.php';
    
    // Create the subcategory page content
    $content = '<?php
session_start();
require_once \'db.php\';
...
<?php include \'footer.php\'; ?>
    ...
    echo "<p>Created: <a href='$filename'>$filename</a> - " . htmlspecialchars($subcat['name']) . "</p>";
}

echo "<h2>All subcategory pages have been created successfully!</h2>";
echo "<p>Total pages created: " . count($subcategories) . "</p>";
?>
